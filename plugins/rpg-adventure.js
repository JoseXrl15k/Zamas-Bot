...
let handler = async (m, { conn }) => {
    ...
    if (user.health < 80) {
        return conn.reply(m.chat, '💔 No tienes suficiente salud para aventurarte. Usa el comando .heal para curarte.', m, rcanal);
    }
    if (user.lastAdventure && new Date() - user.lastAdventure <= 1500000) {
        let timeLeft = 1500000 - (new Date() - user.lastAdventure);
        return conn.reply(m.chat, `⏳ Estás en cooldown. Espera ${msToTime(timeLeft)} antes de aventurarte de nuevo.`, m, rcanal);
    }
    let kingdoms = [
        'Reino de Eldoria',
        'Reino de Drakonia',
    ...
    let cookies = pickRandom([20, 5, 7, 8, 88, 40, 50, 70, 90, 999, 300]);
