...
🔥 Uso:* ${usedPrefix + command} 1234 Gracias por tu confesión.`);
    ...
    if (!conn.menfess || !conn.menfess[id]) {
        throw m.reply(`*
🔥 Error:* No se encontró ningún mensaje con el ID *${id}*.`);
    }
    
    let { dari, penerima } = conn.menfess[id];
    
    if (m.sender !== penerima) throw m.reply('
🔥 No tienes permiso para responder a este mensaje.');
    ...
    try {
        ...
        let sentMessage = await conn.sendMessage(dari, {
            ...
            contextInfo: {
                mentionedJid: [dari],
                externalAdReply: {
                    title: 'R E S P U E S T A - H O R I E',
                    body: '¡Gracias por usar el servicio de confesiones!',
                    mediaType: 1,
                    renderLargerThumbnail: true,
                    thumbnailUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRIyz1dMPkZuNleUyfXPMsltHwKKdVddTf4-A&usqp=CAU',
                    sourceUrl: 'https://whatsapp.com/channel/0029VayEjZ0IyPtaZAfZeY0P'
                }
            }
        });
