    ...
    return conn.relayMessage(jid, prep.message, { messageId: prep.key.id })
        ...
        getFile: {
            ...
            async value(PATH, saveToFile = false) {
                ...
                if (data && saveToFile && !filename) (filename = path.join(__dirname, '../Zamasu/tmp/' + new Date * 1 + '.' + type.ext), await fs.promises.writeFile(filename, data))
                return {
                    res,
                    filename,
                    ...type,
    ...
    async value(jid, button, rows, quoted, options = {}) {
     ...
     if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = buffer }
          ...
          forwardedNewsletterMessageInfo: {
            newsletterJid: '120363370610108637@newsletter',
            newsletterName: 'ZamasuBot ¡New Channe! 🔥', 
            serverMessageId: '' }, 
                ...options
                }
                })
